...
import SmallBasket from './components/SmallBasket.vue'
import { basketStore } from './store/basket'

const basket = basketStore()
let accessKey = localStorage.getItem('accessKey')
if (!accessKey) {
  axios.get(config.apiUrl + '/users/accessKey').then((rez) => {
    console.log(rez)
    if (rez.data.accessKey) {
      accessKey = rez.data.accessKey
      basket.accessKey = rez.data.accessKey
      localStorage.setItem('accessKey', rez.data.accessKey)
    }
  })
}
basket.get()
